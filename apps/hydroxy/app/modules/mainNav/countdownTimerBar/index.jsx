...
const CountdownTimerBar = ({enable, deadline, copy, bgColor}) => {
  ...
  useEffect(() => {
    const startIt =
      enable && !sessionStorage.getItem(STORAGE_KEY) && !isExpired();
...
const CountdownTimerBarView = ({copy, timer, hideIt, bgColor, showIt}) => {
  const Copy = () => <div className={'copy'}>{copy}</div>;

  const Timer = () => (
    <div className={'timer'}>
      {Object.keys(timer).map((value) => (
        <Fragment key={value}>
          <div className={'group'}>
  ...
  return (
    <div
      id="countdownClock"
      className={'countdownTimerContainer'}
      style={{backgroundColor: bgColor, display: showIt ? 'flex' : 'none'}}
    >
