...
import {comparisonUtils} from '../plp/comparisonModal';
...
const Button = ({product, opensBlank = false, ...rest}) => {
  ...
  return hasVariants ? (
    ...
    <PDPAddToCart
      addItem={{
        variantId: `gid://shopify/ProductVariant/${product.variants[0].id}`,
        slug: product.handle,
        action: 'ADD_TO_CART',
        quantity: 1,
        product,
...
const RebuyProductBox = ({
  ...
  const noPromo = productTags.find((tag) => tag.toLowerCase() === 'no-promo');

  const mainImg = media[0]?.src + '&height=528';
  const secImg = media[1]?.src && media[1]?.src + '&height=528';
          ...
          products: [
            {
              ...
              variant: `${getIdFromGid(product?.variants[0]?.id)}`,
              quantity: 1,
            },
          ],
        },
  ...
  function getPrice() {
    ...
    const gotDifferentPrices = !variants.every(
  ...
  const getPromoPrice = () => {
    let newPrice = 0;
    const price = parseFloat(variants[0]?.price);
    const originalPrice = parseFloat(variants[0]?.compareAtPrice);
  ...
  const PriceComp = () => {
    const price = parseFloat(variants[0]?.price);
    const originalPrice = parseFloat(variants[0]?.compare_at_price);
...
export default RebuyProductBox;
...
const PLPBadges = ({product}) => {
  ...
  const showHolidayBadge = productTags.some(
    (tag) =>
      tag
        .toLowerCase()
        .split('badge:')[1]
