...
const NavCollectionCarousel = ({ collection, navItem }) => {
  ...
  const setStyle = (position) => {
    document.querySelectorAll('.navCollectionWrap').forEach((item, index) => {
      if (carouselWidth === 0) {
        carouselWidth = item.scrollWidth - document.querySelectorAll('.navCollection')[index].offsetWidth;
      }
      if (position > 0 || (Math.abs(position) >= carouselWidth + 1 && position !== 0)) return;
  ...
  useEffect(() => bindCustomEvent(collectionRef, 'data-hover-state', {
    hidden: 'hidden',
    visible: 'visible'
  }));

  return (
    <div className={'navCollectionCarousel hidden navDropDown'} id='nav_click_desktop_t2' ref={collectionRef} data-hover-state="hide">
      <div className={'leftArrow'} onClick={goLeft}>
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="22" viewBox="0 0 30 22" fill="none">
          <path d="M9.78765 21.618C9.97866 21.8477 10.2587 21.986 10.5576 21.9988C10.8564 22.0129 11.147 21.9004 11.3591 21.6883C11.5701 21.4773 11.6826 21.1867 11.6697 20.8879C11.6556 20.589 11.5174 20.309 11.2877 20.1179L3.63408 12.455L28.9301 12.455C29.4973 12.4257 29.9414 11.957 29.9414 11.3886C29.9414 10.8215 29.4973 10.3527 28.9301 10.3222L3.63408 10.3222L11.2994 2.66982C11.7001 2.2503 11.7001 1.58934 11.2994 1.16982C11.1013 0.970602 10.8306 0.858092 10.5494 0.858092C10.2681 0.858092 9.99861 0.970591 9.79938 1.16982L0.318774 10.6504L0.319947 10.6492C0.11604 10.8461 2.76543e-05 11.1168 2.76296e-05 11.3992C2.76048e-05 11.6828 0.116042 11.9536 0.319947 12.1492L9.78765 21.618Z" fill="#4C4E56" />
        </svg>
      </div>
      <div className={'navCollectionWrap'}
