...
const SliderAccount = () => {
  ...
  useEffect(() => {
    ...
    const shouldOpen = (sessionStorage.getItem(openedStateID) === curSliderPanelID);
...
const MainContent = () => {
  ...
  function tooglePasswordType(elementRef) {
    ...
    elementRef.current.setAttribute('type', (curValue === 'text') ? 'password' : 'text');
      ...
      <div className={'pointsDisplay'}>
        {
          (points !== null && points !== undefined)
            ...
            : <LoadingSkeleton width="13px" height="10px" style={{ borderRadius: '6px' }} />
        }
      </div>
  ...
  const Links = () => {
    ...
    const links = [
      { label: 'return & exchanges', to: (getApiKeys().RETURNS_HREF), showIt: true },
      { label: 'faqs & help', to: '/pages/faq', showIt: true },
      { label: 'submit a receipt', to: '/pages/upload-receipt', new: true, showIt: (getApiKeys().FEATURE_FLAGS.LOYALTY), onClick: () => triggerAnalyticsLoyaltyEvents('SubmitReceiptBtnClick', { source: 'accountSlider' }) },
      { label: 'redeem rewards', to: ((isLoggedIn) ? '/account?c=rewards' : '/rewards'), new: true, showIt: (getApiKeys().FEATURE_FLAGS.LOYALTY) },
      ...
      <div id={'links'}>
        {
          links.map((data) => (
            (data.showIt) &&
            <Link to={data.to} onClick={(data.onClick || null)} key={data.label}>
              {data.label}
      ...
      <div className={'welcomeButtonsWrap'}>
        {
          ...
          links.map((data) => (
            ...
            <Link to={data.to} className={'welcomeButton'} onClick={() => switchSliderPanelVisibility('SliderAccount')} key={data.label}>
...

        {
          (getApiKeys().FEATURE_FLAGS.LOYALTY) && (
            <Link to={'/account?c=rewards'} className={'welcomeButton rewards'} onClick={() => switchSliderPanelVisibility('SliderAccount')}>
  ...
  const RewardsBannersSection = () => {
    ...
    return (
      ...
      <div className={'rewardsBannerContainer'} ref={container}>
        <Link to={'/rewards'} onClick={() => triggerAnalyticsLoyaltyEvents('LearnMoreBtnClick', { source: 'accountSlider' })}>
          <span>{icons['round_star']} introducing</span>
          <div>TULA 24-7 Rewards</div>
          <div>Loyalty that glows <br />
            <span><b>Sign up now</b> {icons['arrow']}</span>
          </div>
        </Link>
        <Link to={'/rewards'} className={'hideOnDesktop'} onClick={() => triggerAnalyticsLoyaltyEvents('LearnMoreBtnClick', { source: 'accountSlider' })}>
        ...
        <Link to={'/rewards'} className={'hideOnDesktop'} onClick={() => triggerAnalyticsLoyaltyEvents('LearnMoreBtnClick', { source: 'accountSlider' })}>
          ...
          <div>Redeem your points for full-size <b>products</b>, <b>discounts</b>, and members-only <b>perks</b>.</div>
        </Link>
  ...
  const contents = {
...

      {
        (getApiKeys().FEATURE_FLAGS.LOYALTY) ? (
          <div id={'createNewAccRewards'}>
            <p>
              <span onClick={() => { changeMainContent('createAccount'); triggerAnalyticsLoyaltyEvents('SignupBtnClick', { source: 'accountSlider' }); }}>create an account</span> to manage your orders & auto-deliveries and earn rewards! {icons['arrow']}
            </p>
            <RewardsBannersSection />
          </div>
        ) : (
          <div id={'createNewAcc'}>
            <p>create an account to easily manage your orders and auto-deliveries</p>
            <div id={'createNewAccButton'} onClick={() => { changeMainContent('createAccount'); }}>create an account</div>
          </div>
        )
      }
    ...
    'welcomeBack': (<>
      ...
      <div className={'welcomeBottomButton'} onClick={handleLogout.bind(this)}>sign out</div>
    ...
    'createAccountSuccess': (<>
      ...
      <div id={'createAccountSuccess'}>
        {icons['check']}
        <p>thank you for creating <br />a TULA account!</p>
        <Link className={'button'} to={'/collections/all'} onClick={() => {
          switchSliderPanelVisibility('SliderAccount');
          changeMainContent('signIn');
        }}>
          shop now
        </Link>
        <Link className={'viewMyAccountLink'} to={'/account?c=rewards'}>view my account</Link>
        ...
        <div id={'createAccountBannersContainer'}>
          ...
          <Link className={'skinquizBanner'} to={'/pages/skincare-finder'}>
            <span>skin quiz</span>
            <div>Get your personal <br />skincare routine</div>
            {icons['round_arrow']}
          </Link>
        </div>
