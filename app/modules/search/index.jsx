      ...
      <a href={'/products/' + product.handle} className={'searchProductLink'}>
        ...
        <div className={'productInfo'}>
          {product.alt_title !== undefined &&
            <div className={'productAltSearch'}>
              {product.alt_title}
            </div>
          }
...
const getBannersHeight = () => {
  ...
  if (typeof document !== 'undefined') {
    ...
    const promoBannerWrapHeight = document.querySelector('.promoBannersWrap')?.offsetHeight;
    const topHeaderContainerHeight = document.querySelector('.topHeaderContainer')?.offsetHeight;
  ...
  useEffect(() => {
    ...
    if (state === 'loaded') {
      ...
      const filteredProds = config.searchProducts.map((prods) => {
        return products.filter(product => product.handle === prods.productId)[0];
      });
