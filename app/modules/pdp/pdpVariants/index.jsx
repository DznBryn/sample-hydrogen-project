...
const PDPVariants = ({classes, details = {}}) => {
  ...
  function setStoreInitialValue() {
    function getSelectedVariant() {
      const isVariantsOnUrl =
        searchParams.get('variant') && searchParams.get('variant') !== '';
  ...
  function getVariantTypes(variants = null) {
    ...
    if (variants !== null)
      variants.forEach(
        (variant) =>
          variant.selectedOptions && getTypes(variant.selectedOptions),
      );
...
const VariantsContainer = ({data = null}) => {
  ...
  const {
    variants,
    variants_title: variantsTitle,
    productPromos,
    variant_shade: variantShade,
  } = data;
  const variantsWraperScrollLeft = useRef(0);
  const variantsWraper = useRef(null);

  const TypeShade = store?.productPage?.types?.find(
    (option) => option.name.toLowerCase() === 'shade',
  );
  const TypeSize = store?.productPage?.types?.find(
    (option) => option.name.toLowerCase() === 'size',
  );

  useLayoutEffect(() => {
    if (variantsWraper.current)
      variantsWraper.current.scrollLeft = variantsWraperScrollLeft.current;
  });
  ...
  const ContainerTitle = () =>
    TypeShade ? (
      ...
      <p className={'variant_title text_bold'}>{'Select a size'}</p>
    ) : (
      ''
    );
    ...
    return (
      variant && (
        <div
          ...
          onClick={() => {
            variantsWraperScrollLeft.current =
              variantsWraper.current.scrollLeft;
  ...
  const CustomVariant = () => {
    return (
      variantShade?.shade_pdp_product_name && (
        <a
...
const SizeVariants = ({data}) => {
  ...
  function getCurrentVariantShade(variantId = null) {
    return variantId && variants.find((variant) => variant.id === variantId);
  }
  ...
  function handleVariantChange(size) {
    return (
      ...
      variants.find(
        (variant = null) =>
          variant &&
          variant.title.includes(getShadeName()) &&
    ...
    productPromos?.variantIds.forEach((promoVariant) => {
      if (variants.find((variant) => variant.id === Number(promoVariant))) {
        ...
        const item = variants.find(
          (variant) => variant.id === Number(promoVariant),
        );
  ...
  const SizesWithShades = () => {
    ...
    return (
      <div className={'variants_size_container'}>
        {TypeSize && TypeSize?.values?.length > 0
              ...
              return (
                ...
                >
                  <p className={'option_text'}>
                    <span className={'text_bold'}>{variantSize[0]}</span> -{' '}
                    {variantSize[1]}
                    <br />
                    {productPromos?.showPromo && enablePromo(size) && (
  ...
  return variants ? (
    variants.length > 2 &&
    TypeSize?.values?.length &&
    TypeSize.values.length <= 2 ? (
      <SizesWithShades />
    ) : TypeSize?.values?.length &&
      TypeSize?.values?.length > 1 &&
      TypeSize.values.length <= 3 ? (
      <SizesWithoutShades />
          ...
          <img
            ...
            src={
              variant?.metafields?.find(
                (meta) => meta.key === 'variant-shade-selector-img',
              )?.value
            }
            alt={
              variant?.metafields?.find(
                (meta) => meta.key === 'variant-modal-image-alt',
              )?.value
            }
          />
        </div>
