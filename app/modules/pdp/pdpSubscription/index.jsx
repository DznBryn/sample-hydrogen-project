...
const PDPSubscription = ({ classes, sellingPlans, autoDeliveryInfo }) => {
  ...
  const sellingPlanOptionValue = useRef(0);
  const sellingPlanId = useRef(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const defaultSellingPlanId = 10092590; // 3 Month ID
  const recommendedSellingPlanId = sellingPlans.RecommendedSellingPlan || defaultSellingPlanId;

  const getRecommendedSellingPlan = useCallback(() => {
    const hasSellingPlan = sellingPlans.SellingPlans.some(
      x => x.sellingPlanID === recommendedSellingPlanId,
    );

    if (hasSellingPlan) {
      return (sellingPlanOptionValue.current = recommendedSellingPlanId);
    }
  }, [sellingPlans.RecommendedSellingPlan]);

  const getSellingPlanName = sellingPlan => {
    if (sellingPlan.sellingPlanID === recommendedSellingPlanId)
      return `${sellingPlan.name} (recommended)`;

    return sellingPlan.name;
  };

  useEffect(() => {
    getRecommendedSellingPlan();
  }, []);
              ...
              sellingPlans.AutoDeliveryMessage.limitedTimeBadge ? (
                ...
                >
                  <span className={'offerContainer'}>
                    <b>
                      <Badges message="limited time" type="limited" color="primary" />
                    </b>&nbsp;
                ...
                <label
                  ...
                  onClick={() => handleSellingOption(1, sellingPlanId.current.value)}
                >
                  <b>Auto-delivery:</b> {sellingPlans.AutoDeliveryDiscount}% off & free shipping{' '}
                  <input
                    type="radio"
                    id="auto"
                    name="selling-option"
          ...
          <div className={'form__content_container'}>
            <span>
              ...
              <select
                ...
                onChange={() => handleSellingPlanOption(sellingPlanId.current.value)}
                onClick={() => handleSellingOption(1, sellingPlanId.current.value)}
              >
