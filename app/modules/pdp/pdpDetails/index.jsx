...
import StickyAddToCart, { links as stickyAddToCartStyles } from '../../stickyAddToCart';
import PDPSubscription, { links as pdpSubscriptionStyles } from '../pdpSubscription';
import PDPAddToCartForm, { links as pdpAddToCartFormStyles } from '../pdpAddToCartForm';
import ConcealerVariant, { links as concealerVariantStyles } from '../concealerVariant';
import ShadeFinderVariant, { links as shadeFinderVariantStyles } from '../shadeFinderVariant';
...
const PDPDetails = ({ product, details = null, shadeVariantsOos, concealerImages = null }) => {
  ...
  const shouldRenderSkinFinder = product?.slug === 'radiant-skin-brightening-serum-skin-tint-spf-30';
  const shouldRenderConcealer = product?.slug === 'radiant-skin-brightening-serum-concealer';

  var { certifiedBadges } = product;
  certifiedBadges = certifiedBadges ? certifiedBadges[0]?.badges : [];

  const singleColumnForCertifiedBadges = certifiedBadges?.some(ele => ele?.title.length > 37);
      ...
      return (
        <>
          <div className={'sfContainer'}>
            ...
            <div>
              <ListViewIcon isSelected={viewType === 'LIST'} onClick={() => setViewType('LIST')} />
              <GridViewIcon isSelected={viewType === 'GRID'} onClick={() => setViewType('GRID')} />
            </div>
          </div>
          {shouldRenderConcealer ? (
            <>
              <ConcealerVariant classes={'order_0'} details={details} viewType={viewType} shadeVariantsOos={shadeVariantsOos} concealerImages={concealerImages} />
              <div className={'openSFButton'} onClick={() => switchSliderPanelVisibility('ConcealerSlider')}>find my shade <OpenSFButtonIcon /></div>
            </>) : (
            <>
              <ShadeFinderVariant classes={'order_0'} details={details} viewType={viewType} shadeVariantsOos={shadeVariantsOos} concealerImages={concealerImages} />
              <div className={'openSFButton'} onClick={() => switchSliderPanelVisibility('ShadeFinderSlider')}>find my shade <OpenSFButtonIcon /></div>
            </>
          )}
  ...
  const GridViewIcon = ({ isSelected, onClick }) => (
    <svg className={'viewIcon'} onClick={() => onClick()} width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M5.6 0H0V5.6H5.6V0ZM5.6 8.4H0V14H5.6V8.4ZM8.4 8.4H14V14H8.4V8.4ZM14 0H8.4V5.6H14V0Z" fill={isSelected ? '#47C6D9' : '#4C4E56'} fillOpacity={isSelected ? 1 : .2} />
    </svg>
  );

  const ListViewIcon = ({ isSelected, onClick }) => (
    <svg className={'viewIcon'} onClick={() => onClick()} width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    selectedVariant !== 0
      ? details.variants > 1
        ? details.variants
          ...
          ?.metafields?.find(meta => meta.key === 'variant-modal-text')?.value || ''
        : defaultDescription
      : defaultDescription;
  ...
  const LoyaltyMessage = ({ className }) => {
    ...
    const Copy = () => {
      ...
      const IS_AD = (store?.productPage?.addToCart?.selling_plan_id && store?.productPage?.addToCart?.selling_plan_id !== 0);

      function getPoints() {

        const { price, discount: DISCOUNT_PERCENT, promos } = getPrice(store?.productPage?.selectedVariant ?? 0);

        const MULTIPLIER = 10;
        const ADD_WHEN_AD = 300;
        const SHOWING_PROMO = (promos?.showPromo && promos?.variantIds.includes(store?.productPage?.selectedVariant.toString()));
        ...
        const POINTS = (IS_AD) ? ((Math.round(curPrice) * MULTIPLIER) + ADD_WHEN_AD) : (curPrice * MULTIPLIER);
      ...
      return (
        ...
        <div className={'loyaltyMessageContainer__copy'}>
          {
            (isLoggedIn) ? (
              <span>You’re earning <b>{getPoints()} points</b> with TULA 24-7 Rewards!</span>
            ) : (
              <span>Earn <b>{getPoints()} points</b> with TULA 24-7 Rewards</span>
            )
          }
        </div>
      ...
      <div className={'loyaltyMessageContainer__action'}>
        {
          (isLoggedIn) ? (
            ...
            <div className={'loyaltyMessageContainer__joinButton'} onClick={() => { switchSliderPanelVisibility('SliderAccount'); triggerAnalyticsLoyaltyEvents('SignupBtnClick', { source: 'pdpPoints' }); }}>
              Join now<ArrowIcon />
            </div>
          )
        }
      ...
      return (
        ...
        <ModalGeneric isOpen={isModalOpen} handleClose={handleModalDisplay}>
          <div className={'loyaltyMessageContainer__modalContent'}>
            <InfoIcon />
            <b>Points displayed is based on the product price. Final points you will earn will be calculated in checkout.</b>
          </div>
        </ModalGeneric>
  ...
  return (
    (details && (
      <div className={'details_container'}>
        ...
        <PDPPrice pricing={getPrice(store?.productPage?.selectedVariant ?? 0)} />
        {
          (getApiKeys().FEATURE_FLAGS.LOYALTY) && <LoyaltyMessage className={'show_mobile'} />
        }
        <PDPDescription
          classes={'order_7'}
        ...
        {
          (!details.tags.find(tag => tag.toLowerCase().replace(' ', '-') === 'gift-card') && certifiedBadges?.length > 0) && (
            ...
            <div className={'ctaContainer'}>
              ...
              <PDPAddToCart
                ...
                addItem={{
                  ...
                  ['selling_plan_id']: store?.productPage?.addToCart?.selling_plan_id &&
