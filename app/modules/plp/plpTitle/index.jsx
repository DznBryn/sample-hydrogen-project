...
const PLPAfterpay = ({ classes }) => (
  <div className={classnames('plpTitle_afterpay_container', classes)}>
    <IconAfterpay />
    <p className={'afterpay_text'}>Available for orders over {getCurrency()}35</p>
  </div>
);
...
const EyeQuizLinkBanner = () => (
  <div className={'linkBannerContainer'}>
    <a href={'/eyecare-finder'} className={'linkBannerText'} target="_self">find your perfect eye routine <RightArrow /></a>
  </div>
);

const PLPTitle = ({ title, showAfterpay }) => {
  const showEyeQuizLinkBanner = (title.title.toLowerCase().includes('eye care') && getApiKeys().CURRENT_ENV.includes('US'));
  return (
...
const IconAfterpay = () => (
  <svg version="1.1" id="Afterpay_Logo_Black" className={'afterpay_logo'} x="0px" y="0px" viewBox="0 0 1165.5 234.5" >
    <g>
