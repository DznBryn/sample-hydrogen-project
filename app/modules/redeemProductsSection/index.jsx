...
const RedeemProductsSection = ({ content }) => {
  const { products = mock, parentComp } = content;
  ...
  return (
    <div className={styles.redeemProductsListContainer}>
      <Swiper
        ref={swiperRef}
        observer={true}
        observeParents={true}
      ...
      >
        {products.map((item) => (
          <SwiperSlide key={item._id}>
            <div className={styles.redeemProductItem}>
              <YotpoProductBox yotpoProduct={item} parentComp={parentComp}/>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
...
const YotpoLoginButton = () => (
  <button
    className={styles.yotpoLoginButton}
    type="button"
    onClick={() => {
