  ...
  useEffect(() => {
    ...
    document.addEventListener('scroll', function () {
      ...
      if (scrollPosition < oneHundredVh) {
        document.getElementById('imgOne').style.opacity =
          1 - scrollPosition / window.innerHeight;
        document.getElementById('colorOne').style.opacity =
          1 - scrollPosition / window.innerHeight;
      }

      // Second Image Transition
      if (document.getElementById('imgOne').style.opacity < 0.1) {
        document.getElementById('imgTwo').style.opacity =
          1 - (scrollPosition - window.innerHeight) / window.innerHeight;
        document.getElementById('colorTwo').style.opacity =
          1 - (scrollPosition - window.innerHeight) / window.innerHeight;
      }

      // Third Image Transition
      if (document.getElementById('imgTwo').style.opacity < 0.1) {
        document.getElementById('imgThree').style.opacity =
          1 - (scrollPosition - window.innerHeight * 2) / window.innerHeight;
        document.getElementById('colorThree').style.opacity =
          1 - (scrollPosition - window.innerHeight * 2) / window.innerHeight;
      }
      ...
      if (
        scrollPosition / oneHundredVh > positionOnSlide &&
        scrollPosition / oneHundredVh <= 1
      ) {
        if (!(count >= 2)) {
          setCount(2);
        }
      } else if (
        scrollPosition / (oneHundredVh * 2) > positionOnSlide &&
        scrollPosition / (oneHundredVh * 2) <= 1
      ) {
        if (!(count >= 3)) {
          setCount(3);
        }
      } else if (
        scrollPosition / (oneHundredVh * 3.5) > positionOnSlide &&
        scrollPosition / (oneHundredVh * 3.5) <= 1
      ) {
        if (!(count >= 3)) {
          setCount(4);
        }
      ...
      <div id="slidesWrapper" className={'section right'}>
        <div className={'slide'}>
          ...
          <div className={'scrollDown'}>
            ...
            >
              <path
                d="M9 8.93746L0.770996 1.12143L1.51476 0.414856L8.99996 7.5258L16.4852 0.414856L17.2289 1.12143L9 8.93746Z"
                fill="#4C4E56"
              />
            </svg>
          </div>
        ...
        <div className={'slide'}>
          <div id="slideCopyTwo" className={'copy.active fadeIn'}>
            ...
            <p className={'fadeInParagraph'}>
              TULA founder, Dr. Roshini Raj is a practicing gastroenterologist
              who has been studying probiotics for 20 years. A breakthrough
              innovation in skincare, probiotic extracts are a powerful
              ingredient that help soothe, hydrate and nourish skin to improve
              its balance and calm the look of irritation. That&apos;s why 100%
        ...
        <div className={'slide slideCopyThree'}>
          <div className={'copy.active fadeIn'}>
            ...
            <p className={'fadeInParagraph'}>
              We create hardworking formulas made with good-for-you ingredients
              so our community can feel confident in choosing TULA products. At
        ...
        <div className={'slide longerSlide'}>
          <div id="slideCopyFour" className={'copy.active fadeIn slideFour'}>
            <h2 className={'fadeIn'}>#EmbraceYourSkin</h2>

            <p className={'fadeInParagraph'}>
              Our vision is to inspire confidence. We embrace skin positivity
              and approach beauty from the inside out. That&apos;s why we
              promise to reimagine social norms around beauty standards and to
              never retouch any photos of skin. You&apos;ll see “Untouched
              Beauty” badges on all of our photos to solidify this promise.
            </p>

            <p className={'hideParaTwo showParaTwo'}>We Promise to</p>
            ...
            <p className={'hideParaFive showParaFive'}>REIMAGINE language.</p>

            <p className={'hideParaSix showParaSix'}>
              Ageless is the new anti-aging.&#8482;
            </p>

            <p className={'hideParaSeven showParaSeven'}>REIMAGINE imagery.</p>
