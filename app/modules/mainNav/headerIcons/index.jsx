...
const HeaderIcons = ({cartConfig, hideSearch, fixedRight, lpMinimalHeader}) => {
    ...
    if (cartTotal >= cartPageConfig?.freeShippingThreshold) {
      ...
      progressMsg = (
        <p>
          ...
          {getCurrency() +
            (
              cartPageConfig?.freeShippingThreshold - parseFloat(cartTotal)
            ).toFixed(2)}{' '}
  ...
  return (
    <>
      <div
        id={lpMinimalHeader ? 'cartAlertMinimalHeader' : '___cartAlert___'}
        className={'cartAlert hidden'}
        ref={alertRef}
        data-alert-state="hide"
      >
      ...
      {fixedRight === true ? (
        <ul id={'headerIcons'} className={'header_icons'}>
          <li className={classname('icon__container', 'account')}>
            <AccIcon />
          </li>
          {hideSearch !== true ? (
            <li className={classname('icon__container', 'search')}>
              <IconSearch />
            </li>
          ) : null}
