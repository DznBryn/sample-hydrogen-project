...
const AnnouncementTopBanner = ({content}) => {
  ...
  useEffect(() => {
    if (window) {
      const bannerVisibility = window?.sessionStorage.getItem(
        'annoucementBannerVisibility',
      );
  ...
  return (
    <div
      className={showBanner ? 'bannerContainer' : 'hideBanner'}
      style={{background}}
    >
      <PortableText value={messageRaw} />
      <span style={{color: closeButtonColor}} onClick={removeBanner}>
        +
      </span>
    </div>
  );
