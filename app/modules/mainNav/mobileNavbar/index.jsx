...
const defaultThumbnail =
  'https://res.cloudinary.com/shogun-frontend/image/fetch/f_auto,q_auto,c_limit,w_1920/https://f.shgcdn.com/1cf676c2-5a99-401a-9d1d-7b84ebf9a666/';
...
const MobileNavbar = ({content}) => {
  ...
  function resetSubItemsScroll() {
    subItemsContainer.current?.scrollTo(0, 0);
  }
  ...
  const renderIcon = (img) => <img className={'icon'} src={img} />;
            ...
            <div className={'mobileSubNavWrapper'} ref={subItemsContainer}>
              {navbarContent[subItemsOpenId].navLinks?.map((subItem, idx) => (
                <a
                  ...
                  key={subItem.displayText}
                >
                  <div key={idx} className={'mobileSubNavItem'}>
                    <img
                      src={
                        subItem.thumbnail
                          ? subItem.thumbnail.asset.url + '?auto=format'
                          : defaultThumbnail
                      }
                      className={'mobileSubNavImage'}
                    />
                    <span
                      style={
                        subItem.fontColorHex
                          ? {color: subItem.fontColorHex}
                          : {}
                      }
                    >
                      {subItem.emoji && (
                        <img
                          className={'emoji'}
                          src={subItem.emoji.asset.url + '?auto=format'}
                        />
                      )}
                      {subItem.displayText.toLowerCase()}
                    </span>
                    {subItem.calloutText && (
