...
const SMSSignUp = ({content}) => {
	...
	const handleSubmit = async (e) => {
      ...
      .catch((error) => console.log('Not able to create postscript customer with the phone', phoneNumber, 'Error', error));
	}

	useEffect(() => {
		if(window.document){
				if(document.querySelector("script[src='https://sdk.postscript.io/sdk.bundle.js?shopId=273330']")){
					let addPSScript = setInterval(()=>{
						document.querySelector("script[src='https://sdk.postscript.io/sdk.bundle.js?shopId=273330']").remove()
						clearInterval(addPSScript)
					}, 200)
				} 
				appendScript("https://sdk.postscript.io/sdk.bundle.js?shopId=272499")?.then(() => {})
		}
  }, [])

	useEffect(() => {
		if(typeof _ltk === "undefined") {
			appendScript('https://cdn.listrakbi.com/scripts/script.js?m=4aBATcUCndfp&v=1')?.then(() => {
					return true;
			});
		}
	}, [])
	...
	return(
		<div id="sms-signup" className={"container"}>
			<div id="sms-page" className={"content"}>
				<div 
					className={"section"} 
					style={{backgroundImage: `url(${content[0].backgroundImage.asset.url})`}}
				>
			...
			>
				<div className={"closeButton"} onClick={() => setIsModalOpen(false)}>+</div>
