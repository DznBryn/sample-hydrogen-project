...
const OOSItem = ({ gridView }) => (
  <svg width="37" height="37" viewBox="0 0 37 37" fill="none" style={{ marginRight: gridView ? 0 : 10 }} xmlns="http://www.w3.org/2000/svg">
    <g clipPath="url(#clip0_1143_8013)">
      <path d="M35 17.5C35 20.9612 33.9736 24.3446 32.0507 27.2225C30.1278 30.1003 27.3947 32.3434 24.197 33.6679C20.9993 34.9924 17.4806 35.339 14.0859 34.6637C10.6913 33.9885 7.57306 32.3218 5.12564 29.8744C2.67822 27.427 1.01151 24.3088 0.336265 20.9141C-0.338976 17.5194 0.00758243 14.0007 1.33212 10.803C2.65665 7.60534 4.89967 4.87221 7.77753 2.94928C10.6554 1.02636 14.0388 2.86067e-06 17.5 2.86067e-06C19.7985 -0.0013117 22.0747 0.450442 24.1985 1.32943C26.3223 2.20843 28.252 3.49742 29.8773 5.1227C31.5026 6.74799 32.7916 8.6777 33.6706 10.8015C34.5496 12.9253 35.0013 15.2015 35 17.5Z" fill="#65442E" fillOpacity="0.3" />
...
const PDPVariants = ({ details = {}, viewType, shadeVariantsOos = [] }) => {
  ...
  const handleRecommendation = (shadeRecommended) => {
    ...
    const variant = details?.variants?.find(variant => variant.name.split(' ')[1] === shadeNumber);
  ...
  const ListViewVariants = (shadeRecommended, shadeBackground, selectedShade) => {
    ...
    if (isOOSVariant) {
      return (
        <div className={'sfVariantItem'} onClick={() => handleRecommendation(shadeRecommended)} name={shadeRecommended} id={shadeRecommended} key={shadeRecommended} style={{ color: 'rgba(76, 78, 86, 0.2)' }}>
          <OOSItem />
          {shadeRecommended}
  ...
  const GridViewVariants = (shadeRecommended, shadeBackground) => {
    ...
    if (isOOSVariant) {
      return (
        <div className={'sfVariantItem'} onClick={() => handleRecommendation(shadeRecommended)} key={shadeRecommended}>
          <OOSItem gridView />
          {isSelected === shadeRecommended && <Selected oos />}
        </div>
      );
          ...
          (
            <div className={'sfVariantsContainer'} ref={variantsWraper} id="sfVariantsContainer">
              {mockedShadeResult.map(({ shadeRecommended, shadeBackground }) => {
                return <>{renderVariants(shadeRecommended, shadeBackground, selectedShade)}</>;
              })}
            </div>
          ) :
          (
