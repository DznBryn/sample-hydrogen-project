...
export default function AddToCartButton({
  ...
  const { quantity, availableForSale } = useInventory({ id: lineItem?.variantId, slug: lineItem?.slug });
  ...
  const lineItems = [{ merchandiseId: lineItem?.variantId, quantity: 1 }];

  useEffect(() => {
    console.log(quantity, addToCart);
    if (quantity === 0 || !availableForSale || forceSoldOut) {
      setButtonState(SOLD_OUT);
    } else if (isGated && !isLoggedIn) {
      setButtonState(LOCKED);
    } else if (lineItem?.variantId && lineItem?.variantId === 0) {
      setButtonState(SELECT_SHADE);
    } else if (addToCart?.state === 'submitting' || addToCart?.state === 'loading') {
  ...
  const mapStateToButton = {
    ...
    [LOADING]: <div className="addToCart__container">
      <button
        className={`${classes.join(' ')} add_to_cart disabled`}
        style={
