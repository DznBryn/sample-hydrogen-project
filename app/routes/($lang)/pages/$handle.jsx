...
import InfluencerPage, { links as influencerPageStyles } from '~/modules/influencerPage';
import { GET_CART_PAGE_CONFIG, GET_INFLUENCER_PAGE } from '~/utils/graphql/sanity/queries';
import { getCMSContent, getCMSDoc, getCMSProductsWithShopifyData, getPageOnCMSBySlug } from '~/utils/functions/eventFunctions';

export const links = () => {
  return [
    ...influencerPageStyles()
  ];
};
...
export const loader = async ({ params, context }) => {
  ...
  const influencerPagesOnCMS = await getCMSContent(context, GET_INFLUENCER_PAGE);
  const influencerPage = getPageOnCMSBySlug(influencerPagesOnCMS, handle);

  if (!influencerPage) throw new Response(null, {status: 404, statusText: 'Not Found',});

  /**
   * if the handle matches with any influencer page on CMS...
   */

  const { collection: shopifyInfluencerCollection } = await context.storefront.query(PRODUCTS_QUERY, { variables: { handle: influencerPage.plpCollection.collectionId } });
  ...
  if(influencerPage.plpCollection.collectionId !== 'all'){
    ...
    const { collection: shopifyAllCollection } = await context.storefront.query(PRODUCTS_QUERY, { variables: { handle: 'all' } });
  ...
  return json({
    pageContent: {
      ...influencerPage,
      influencerProducts: getCMSProductsWithShopifyData(influencerPage.influencerProducts, allCollection),
      plpCollection: {
