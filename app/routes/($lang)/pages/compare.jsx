...
import { getCMSContent, getCollectionWithCMSData } from '~/utils/functions/eventFunctions';
...
export async function loader({ request, context }) {
  ...
  if (collectionHandle !== null) {
    ...
    collectionsCMSData = await getCMSContent(context, GET_PRODUCT_COLLECTIONS);

    const data = await context.storefront.query(PRODUCTS_QUERY, {variables: {handle: collectionHandle,},});
...
function getFilterProducts(collectionWithCMSData, productsToShow){
  ...
  const filteredProducts = products.filter((value) => productsToShow?.includes(value.handle));
